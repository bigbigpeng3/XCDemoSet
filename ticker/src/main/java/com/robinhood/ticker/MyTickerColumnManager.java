 ...
 * by scrolling the column vertically. The {@link MyTickerColumnManager} is then a
...
class MyTickerColumnManager {
    ...
    MyTickerColumnManager(TickerDrawMetrics metrics) {
     ...
     */
    void draw(Canvas canvas, Paint textPaint, Paint borderPaint, int mPadding) {
        ...
        for (int i = 0, size = tickerColumns.size(); i < size; i++) {
            ...
            column.draw(canvas, borderPaint);

            canvas.translate(column.getCurrentWidth() + mPadding, 0f);
        }
